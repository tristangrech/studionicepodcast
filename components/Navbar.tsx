...
const mobileMenuVariants = {
  hidden: {
    opacity: 0,
    height: 0,
    transition: { duration: 0.3, ease: 'easeInOut' as const, when: 'afterChildren' },
  },
  visible: {
    opacity: 1,
    height: 'auto',
    transition: { duration: 0.3, ease: 'easeInOut' as const, when: 'beforeChildren', staggerChildren: 0.05 },
  },
}
          ...
          <div className="hidden md:flex items-center space-x-6 lg:space-x-8">
            {navLinks.map((link) => (
              <button
                ...
                className="relative text-gray-300 hover:text-accent transition-colors px-3 py-2 text-base font-medium group"
              >
                {t(link.key)}
                <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-accent rounded-full transition-all duration-300 group-hover:w-3/4 group-hover:shadow-[0_0_8px_rgba(212,175,55,0.6)]" />
              </button>
            ))}
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="bg-accent hover:bg-accent-light text-primary font-semibold px-6 py-2.5 rounded-full transition-colors text-base shadow-lg shadow-accent/20"
            >
              {t('bookNow')}
            </motion.button>
            <LanguageSwitcher />
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="px-4 pt-2 pb-6 space-y-2 bg-primary/98 backdrop-blur-md border-t border-white/10">
              ...
              <motion.div variants={mobileItemVariants} className="pt-4 space-y-2">
                <button
                  onClick={handleBookNow}
